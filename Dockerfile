...
ENV JAVA_OPTS="\
 ...
 -XshowSettings:vm\
 -javaagent:/opt/jmx_prometheus_javaagent.jar=3066:/opt/jmx_exporter_config.yml"

ENV APP_HOME=/usr/app/
WORKDIR $APP_HOME

COPY --from=TEMP_BUILD_IMAGE $APP_HOME/build/libs/peppol-mlr-reporter.jar .

HEALTHCHECK --interval=15s --timeout=30s --start-period=40s --retries=15 \
  CMD wget --quiet --tries=1 --spider http://localhost:3042/api/health/check || exit 1
